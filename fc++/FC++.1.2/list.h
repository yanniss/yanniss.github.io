...
class List {
   struct Dummy : public CFunType<Ref<LazyPair<T> > > {
      ResultType operator()() const { return Ref<LazyPair<T> >(0); }
   };
   ...
   template <class It>
   static LazyPair<std::iterator_traits<It>::value_type>::Type
   stl_range( const It& i, const It& end ) {
      typedef std::iterator_traits<It>::value_type U;
      if( i==end )
         return LazyPair<U>::Type(0);
      else {
         It j = i;
         j++;
         LazyPair<U>::Type (*pf)( const It&, const It& ) = &List::stl_range;
         return LazyPair<U>::Type(
                new LazyPair<U>( *i, makeFun0( 
...
struct Cat {
   ...
   operator()( const LLGenA& a, const LLGenB& b ) const {
      ...
      typedef LazyPair<T>::GenFxn GenFxn;
      typedef LazyPair<T>::Type Type;
      GenFxn aa = magic<T>()(a);
      GenFxn bb = magic<T>()(b);
