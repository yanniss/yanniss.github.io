...
public:
   ...
   template <class Arg2>
   struct Sig 
   : public FunType<Arg2,typename Binary::Sig<Arg1,Arg2>::ResultType> {};
...
public:
   ...
   template <class Arg1>
   struct Sig 
   : public FunType<Arg1,typename Binary::template Sig<Arg1,Arg2>::ResultType> {};
...
: public CFunType<typename RT<Ternary,A1,A2>::ResultType> {
   Ternary f;
   A1 a1;
   A2 a2;
public:
   binder1and2of3(const Ternary& w, const A1& x, const A2& y) : 
     f(w), a1(x), a2(y) {}
   template <class Arg3>
   ResultType operator()(const Arg3& z) const { return f(a1,a2,z); }
};
