...
struct Cons; struct Head; struct Tail; struct Null; struct Cat;
...
template <class U,class F>
   struct cvt {
   typedef OddList<U> ResultType;
   struct Sig : public FunType<OddList<U> > {};
   F f;
   cvt( const F& ff ) : f(ff) {}
...
struct Cat {
   ...
   operator()( const L& l, const M& m,
               Reuser2<Inv,Var,Inv,Cat,List<typename L::ElementType>,M>
               r = NIL ) const {
      if( null(l) ) 
         return m();
      else
         return cons( head(l), r( Cat(), tail(l), m ) );
   }
   template <class L, class T>
   OddList<T> operator()( const L& l, const List<T>& m,
                            Reuser2<Inv,Var,Inv,Cat,List<T>,List<T> >
                            r = NIL ) const {
      if( null(l) ) 
         return m;
      else
   ...
   template <class L, class T>
   OddList<T> operator()( const L& l, const OddList<T>& m,
                            Reuser2<Inv,Var,Inv,Cat,List<T>,OddList<T> >
                            r = NIL ) const {
      if( null(l) ) 
         return m;
      else
