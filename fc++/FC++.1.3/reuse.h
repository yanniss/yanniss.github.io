...
template <class V, class X> struct Maybe;
template <class X>
struct Maybe<Var,X> {
...
struct Thunk0 : public Fun0Impl<R> {
   ...
   void init( const F& ff ) const {
      Maybe<V1,F>::remake( f, ff );
   }
   R operator()() const {
      return Maybe<V1,F>::clone(f)( Reuser0<V1,F>(this) );
   }
};
...
struct Thunk1 : public Fun0Impl<R> {
   ...
   void init( const F& ff, const X& xx ) const {
      Maybe<V1,F>::remake( f, ff );
      Maybe<V2,X>::remake( x, xx );
   }
   R operator()() const {
      return Maybe<V1,F>::clone(f)( 
         Maybe<V2,X>::clone(x), 
